...
export function verifyToken(token: string, isRefreshToken = false): DecodedToken | null {
  try {
...
export function generateToken(payload: object, expiresIn: ExpiresIn = '1h', isRefreshToken = false): string {
  const secret = isRefreshToken ? JWT_REFRESH_SECRET : JWT_SECRET;
