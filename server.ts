...
const io = new Server(httpServer, {
  cors: {
    origin: [process.env.NEXT_PUBLIC_URL || 'http://localhost:4000', 'http://localhost:3000', 'https://whispr-o7.vercel.app/'],
...
io.on('connection', (socket: Socket) => {
  ...
  socket.on('joinRoom', async (room: string, userId: string, firstName: string, lastName: string) => {
    ...
    if (previousPublicRoom && previousPublicRoom !== room) {
      ...
      if (prevRoomUsers) {
        ...
        io.to(previousPublicRoom).emit('userLeft', { username: fullName, room: previousPublicRoom });
      }
      if (typingUsers.has(previousPublicRoom)) {
        const fullName = `${socket.firstName} ${socket.lastName}`;
        typingUsers.get(previousPublicRoom)!.delete(fullName);
        io.to(previousPublicRoom).emit('userStoppedTyping', { username: fullName, room: previousPublicRoom });
      }
    }
    ...
    socket.to(room).emit('userJoined', { username: fullName, room });
  ...
  socket.on('joinPrivateRoom', async ({ senderId, senderFirstName, senderLastName, receiverId, receiverFirstName, receiverLastName }: JoinPrivateRoomArgs, callback) => {
    ...
    const fetchReceiverDetails = async (): Promise<{ firstName: string; lastName: string }> => {
      try {
        ...
        if (receiverUser) {
          return { firstName: receiverUser.firstName, lastName: receiverUser.lastName };
        }
  ...
  socket.on('sendMessage', async ({ id, userId, firstName, lastName, text, room, fileUrl, fileType, fileName, replyTo }: SendMessageArgs) => {
    if (!text && !fileUrl) {
  ...
  socket.on('privateMessage', async ({ id, senderId, senderFirstName, senderLastName, receiverId, receiverFirstName, receiverLastName, text, fileUrl, fileType, fileName, replyTo }: PrivateMessageArgs, callback) => {
    if (!text && !fileUrl) {
        ...
        for (const receiverSocketId of receiverSocketIds) {
          ...
          if (receiverSocket && !receiverSocket.rooms.has(privateRoomId)) {
            receiverSocket.emit('privateMessageNotification', {
              senderId,
              senderUsername: senderFullName,
              messageSnippet: notificationContent.length > 100 ? notificationContent.substring(0, 97) + '...' : notificationContent,
  ...
  socket.on('getPrivateMessages', async ({ user1Id, user2Id }: GetPrivateMessagesArgs) => {
    ...
    try {
      ...
      const formattedMessages = messages.map((m) => ({
        ...
        receiverUsername: m.receiverFirstName && m.receiverLastName ? `${m.receiverFirstName} ${m.receiverLastName}` : undefined,
  ...
  socket.on('editMessage', async ({ messageId, newText, userId }: EditMessageArgs) => {
    try {
      ...
      const fullReceiverName = message.receiverFirstName && message.receiverLastName ? `${message.receiverFirstName} ${message.lastName}` : undefined;
      ...
      if (updatedMessageData.chatType === 'room' && updatedMessageData.room) {
        ...
        const privateRoomId = getPrivateRoomId(updatedMessageData.senderId || '', updatedMessageData.receiverId || '');
  ...
  socket.on('typing', ({ room, firstName, lastName, senderId, receiverId }: TypingArgs) => {
    ...
    if (room) {
      ...
      socket.to(privateRoomId).emit('userTyping', { username: fullName, senderId, receiverId });
    }
  });
  ...
  socket.on('stopTyping', ({ room, firstName, lastName, senderId, receiverId }: TypingArgs) => {
    ...
    if (room) {
      ...
      if (typingUsers.has(privateRoomId)) {
        ...
        socket.to(privateRoomId).emit('userStoppedTyping', { username: fullName, senderId, receiverId });
      }
    }
  });
    ...
    if (users.has(socket.id)) {
      ...
      if (currentRoom) {
        ...
        if (roomUsers) {
          ...
          io.to(currentRoom).emit('userLeft', { username: fullName, room: currentRoom });
        }
        if (typingUsers.has(currentRoom)) {
          typingUsers.get(currentRoom)!.delete(fullName);
          io.to(currentRoom).emit('userStoppedTyping', { username: fullName, room: currentRoom });
        }
      }
    }
    ...
    if (socket.userId && userSockets.has(socket.userId)) {
      ...
      socket.activeRooms.forEach(room => {
        ...
        if (typingUsers.has(room)) {
          ...
          io.to(room).emit('userStoppedTyping', { username: `${socket.firstName} ${socket.lastName}`, room });
        }
      });
    }
  });
